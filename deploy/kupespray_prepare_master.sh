...
echo '  cp -rfp inventory/sample inventory/mycluster'
echo "  declare -a IPS=($IPS)"
echo '  CONFIG_FILE=inventory/mycluster/hosts.yml python3 contrib/inventory_builder/inventory.py ${IPS[@]}'
echo ''
echo '  # Review and change parameters under ``inventory/mycluster/group_vars``'
echo '  vim inventory/mycluster/hosts.yml'
echo '  vim inventory/mycluster/group_vars/all/all.yml'
echo '  vim inventory/mycluster/group_vars/k8s-cluster/k8s-cluster.yml'
echo '  vim inventory/mycluster/group_vars/k8s-cluster/addons.yml'
echo ''
...
echo '  ansible-playbook -i inventory/mycluster/hosts.yml --become --become-user=root cluster.yml'
echo ''
